...
int Telemetry_SendRegister(uint8_t reg) {
	...
	return CAN_SendMessage((2 << 8) | my_address, payload, reg_len + 1);
}

void Telemetry_Loop(void) {
	if (slave_faults.byte && global_state.flags.global_arm) {
		if (HAL_GetTick() - last_start_fault_alert > FAULT_SEND_INTERVAL) {
			last_start_fault_alert = HAL_GetTick();
			Telemetry_SendRegister(SLAVE_FAULTS_REG);
		}
	} else {
		last_start_fault_alert = 0; // reset if no faults
	}
	if (on_reg_index < registry_count) {
		uint8_t result = Telemetry_SendRegister(on_reg_index);
		if (result == 0) {
